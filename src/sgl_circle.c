...
void sgl_draw_circle(sgl_screen_t *scr, int32_t x, int32_t y, int32_t d,
    ...
    if (d < 0)
        d = -d;
    if (sgl_check_area(&scr->drawable_area, x, y, x + d, y + d))
        return;
    if (d == 2) {
...
void sgl_draw_circle_center(sgl_screen_t *scr, int32_t xc, int32_t yc,
    ...
    if (r < 0)
        r = -r;
    if (sgl_check_area(&scr->drawable_area, xc - r, yc - r, xc + r, yc + r))
        return;
...
void sgl_draw_ellipse(sgl_screen_t *scr, int32_t xc, int32_t yc, int32_t rx,
    ...
    if (ry < 0)
        ry = -ry;
    if (sgl_check_area(&scr->drawable_area, xc - rx, yc - ry, xc + rx, yc + ry))
        return;
